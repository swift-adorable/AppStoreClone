...
extension Project {
    ...
    public static func makeModule(name: String,
                                  product: Product,
                                  package: [Package] = [],
                                  includeSource: Bool = true,
                                  includeResource: Bool = false,
                                  dependencies: [ProjectDescription.TargetDependency]) -> Project {
    ...
    public static func makeFeatureModule(name: String,
                                         package: [Package] = [],
                                         dependencies: [TargetDependency],
                                         includeInterface: Bool,
                                         includeDemo: Bool) -> Project {
        ...
        let mainScheme = Scheme.scheme(name: name,
                                       buildAction: .buildAction(targets: ["\(name)"]))
        
        schemes.append(mainScheme)
        
        if includeInterface {
            let interfaceTarget = Target.target(name: "\(name)Interface",
                                                destinations: .iOS,
                                                product: .framework,
        ...
        if includeDemo {
            let demoTarget = Target.target(name: "\(name)Demo",
                                           destinations: .iOS,
                                           product: .app,
...
//                                                          .SPM.snapKit
            ...
            let demoScheme = Scheme.scheme(name: name + "Demo",
